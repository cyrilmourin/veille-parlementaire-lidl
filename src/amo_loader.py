...
# P5 (2026-04-25) — cache parallèle texte_ref → libelles_haystack.
...
# ---------------------------------------------------------------------------
# P5 — cache texte_ref → libelles_haystack (cumul libellés d'actes dossier)
# ---------------------------------------------------------------------------
...
def _load_lib_cache() -> dict:
    """Charge le cache texte→libelles_haystack, thread-safe lazy."""
    global _lib_loaded
    with _lib_lock:
        if _lib_loaded is not None:
            return _lib_loaded
        target = _resolve_lib_path()
        if not target.exists():
        ...
        try:
            ...
            log.warning("Cache texte→libelles corrompu : %s", exc)
            _lib_loaded = {"textes": {}, "generated_at": None}
            return _lib_loaded
        if "textes" not in data:
            data = {"textes": {}, "generated_at": None}
        _lib_loaded = data
        log.info(
...
def write_texte_libelles_cache(textes: dict[str, str], path: Path | None = None) -> Path:
    """Persiste le mapping `texteLegislatifRef → libelles_haystack`.

    Appelé par `_normalize_dosleg` après ingestion du dump dossiers. Lu par
    `_normalize_amendement` via `resolve_texte_libelles`.
    """
    global _lib_loaded
    ...
    with _lib_lock:
        _lib_loaded = payload
    log.info(
        "Cache texte→libelles écrit : %d entrées → %s",
        len(textes), target,
    )
    return target
...
def resolve_texte_libelles(texte_ref: str) -> str:
    """Renvoie le haystack cumulé des libellés d'actes du dossier parent.

    Utilisé par `_normalize_amendement` pour enrichir raw.libelles_haystack
    de l'amendement avec le contenu procédural du dossier (P5).
    """
    if not texte_ref or not isinstance(texte_ref, str):
        return ""
