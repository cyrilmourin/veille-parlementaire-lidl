"""Export JSON + Markdown Hugo pour le site statique veille-lidl.sideline-conseil.fr.
...
def _fix_amendement_row(r: dict) -> None:
    ...
    # Gouvernement (pas d'auteur parlementaire, donc aucune photo portrait
    # disponible), on pose un placeholder `/logo_Gouv.png`. Détection :
...
def _write_home(content_dir: Path, rows: list[dict], by_cat: dict[str, list[dict]],
    ...
    lines = [
        "---",
        f'title: "Veille Institutionnelle Lidl — {now:%Y-%m-%d}"',
        f'date: {now:%Y-%m-%d}',
        'description: "Veille parlementaire & institutionnelle — grande distribution alimentaire, compte Lidl France."',
        "---",
        "",
    ]
