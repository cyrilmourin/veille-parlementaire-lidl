...
Variables d'environnement :
    ...
    DIGEST_TO           — destinataire (défaut : ANOUCK.PAUMARD@lidl.fr)
    SITE_URL            — URL publique (défaut : https://veille-lidl.sideline-conseil.fr)
"""
...
DEFAULT_TO = os.environ.get("DIGEST_TO", "ANOUCK.PAUMARD@lidl.fr")
DEFAULT_SITE_URL = os.environ.get("SITE_URL", "https://veille-lidl.sideline-conseil.fr")
...
# Instance Lidl : aucune source "100 % compte client" n'existe. Toutes
# les sources doivent passer le filtre de mots-clés, sinon on inonderait
# le digest. La whitelist R25-H est donc vidée ; on garde la mécanique
# pour éventuellement la réactiver sur un besoin futur (ex. ajouter
# l'Autorité de la concurrence en flux complet).
BYPASS_KEYWORDS_SOURCES: set[str] = set()
...
def run(since_days: int = 1, send: bool = True, verbose: bool = False) -> int:
    ...
    if send:
        subject = f"Veille parlementaire Lidl — {datetime.now():%Y-%m-%d} ({total} nouveautés)"
