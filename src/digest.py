...
<html lang="fr">
<head><meta charset="utf-8"/><title>Veille parlementaire Lidl — {{ date }}</title></head>
<body style="font-family:-apple-system,Segoe UI,Arial,sans-serif;color:#1b2235;background:#F4F6F8;margin:0;padding:24px;">
  <table role="presentation" width="100%" style="max-width:720px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #dfe4ea;">
    <tr><td style="padding:22px 28px;background:#0050AA;color:#F4F6F8;border-bottom:4px solid #E60A14;">
      <div style="font-size:12px;letter-spacing:3px;opacity:.85;font-weight:600;">SIDELINE CONSEIL</div>
      <div style="font-size:22px;font-weight:800;margin-top:3px;color:#fff;">Veille parlementaire Lidl</div>
      <div style="font-size:13px;font-style:italic;color:#E60A14;margin-top:3px;">Voir clair. Jouer juste.</div>
    ...
    {% if total == 0 %}
      ...
      la veille reste active sur <a href="{{ site_url }}" style="color:#E60A14;">{{ site_url }}</a>.</p>
    {% endif %}
    {% for cat, label in categories %}
      {% if cat in buckets %}
      <h2 style="font-size:13px;text-transform:uppercase;letter-spacing:2px;color:#0050AA;margin:26px 0 12px;border-left:4px solid #E60A14;padding:2px 0 2px 10px;font-weight:700;">
      ...
      {% for it in buckets[cat] %}
        ...
        <div style="margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid #e5eaef;">
          {% if it.url %}<a href="{{ it.url }}" style="color:#0050AA;font-weight:600;font-size:15px;text-decoration:none;">{{ it.title }}</a>{% else %}<span style="color:#0050AA;font-weight:600;font-size:15px;">{{ it.title }}</span>{% endif %}
          <div style="color:#5c6577;font-size:12px;margin-top:4px;line-height:1.7;">
            ...
            {% endif %}
            {% if it.published_at %}<span style="color:#E60A14;font-weight:700;font-variant-numeric:tabular-nums;">{{ it.published_at[:10] }}</span>{% endif %}
            {% if it.matched %}
          ...
          {% if it.snippet %}
          <div style="color:#5c6577;font-size:13px;margin-top:8px;padding:8px 12px;border-left:3px solid #E60A14;background:#fff9d6;font-style:italic;">« {{ it.snippet }} »</div>
          {% elif it.summary %}
    ...
    <tr><td style="padding:16px 28px;background:#0050AA;color:#F4F6F8;font-size:12px;text-align:center;">
