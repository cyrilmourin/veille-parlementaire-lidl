...
- **Surveiller le run #41** lancé le 2026-04-26 soir sur `3657ac5` (`reset_db=1`, `since_days=180`, `no_email=1`) — propage 4 fixes en un :
  1. `a2881b9` : haystack CR AN 10 k → 200 k
  2. `ae273ee` : workflow `reset_db=1` purge aussi `data/an_cr_state.json`
  3. `14f0bc6` : senat dosleg uid composite (slug URL) + status_label
  4. `3657ac5` : snippet centré sur 1er matched_keyword (port Bump R40-I sport)

  Vérifier sur le site :
  - `/items/comptes_rendus/` : séances plénières AN + réunions de commission AN matchées (avant fix : 0 sur 6 mois)
  - `/items/dossiers_legislatifs/` : items Sénat avec status_label rempli, URL pointant vers le bon dossier (avant : ppl 2024-247 affiché avec URL ppl77-247.html)
  - Snippets : doivent contenir la phrase qui a déclenché le match, pas un texte générique.

  Runs #34 / #36 / #38 / #39 (reset_db précédents, fixes partiels) sont obsolètes.
...
### Sénat dosleg — `uid` pas unique sur le numéro de texte seul

Bug détecté 2026-04-26 (signalement Cyril : « 2 propositions de loi récentes ont un lien qui ouvre un texte des années 80 »). Le scraper `senat._normalize_rows` utilisait pour `senat_ppl` / `senat_dosleg` / `senat_promulguees` un `uid` extrait de la seule colonne « Numéro de texte » (ou « Numéro de la loi »). Or ce numéro est **recyclé chaque session** parlementaire : ppl.csv contient 9 lignes avec n° 247 (sessions 1977 à 2024), 6 avec 505. `hash_key=senat_ppl::247` partagé → `store.upsert_many` fait `INSERT ON CONFLICT DO UPDATE` qui met à jour title/matched/published_at/raw mais **garde l'`url` immuable** (champ pas listé dans le DO UPDATE SET). La 1re ligne CSV (la plus ancienne) gagne donc l'URL ; les rows suivants n'écrasent que le reste → item affiché avec **titre récent + URL ancienne**.

Fix `14f0bc6` :
1. `uid = slug extrait de l'URL Sénat` (ex. `ppl24-247`) en priorité — toujours unique. Fallback `{session}-{num}` puis `num` seul.
2. `status_label` rempli depuis colonne « État du dossier » du CSV (« caduc », « adopté », « Première lecture (Sénat) »…). Avant : seul `senat_promulguees` posait status_label → cartouche statut vide pour la majorité des dossiers Sénat.

Helper exposé : `senat._extract_dosleg_slug(url)`.
...
- 2026-04-27 : P1d (lexique) + red list + simplification titres CR plénière AN. Signalements Cyril multiples : faux positifs récurrents sur dossiers / CR / amendements / questions, malgré le lexique P1c. Trois leviers actionnés en parallèle :
  1. **Lexique P1d** — retraits du DIRECT (`Autorité de la concurrence`, `Loi ELAN`, `Négociations commerciales`/`Négociation commerciale` en formulation courte) jugés trop transversaux ; `Fournisseurs` retiré entièrement (les vrais textes GD-fournisseurs sont captés par P1a longues + Descrozaille + SRP+10 + EGalim 3) ; resserrement contextuel de `EGalim`/`Egalim`/`E-Galim` (retrait des validateurs ambigus distributeur seul, enseigne seule, négociation commerciale, centrale d'achat, marge commerciale, contrat amont — qui circulent dans l'amont agricole), `Travail dominical`/`Ouverture dominicale` (retrait commerce de détail générique, magasin, enseigne), `DGCCRF` (retrait alimentaire / alimentation / distributeur / enseigne génériques), `Concurrence déloyale` (retrait distributeur / enseigne génériques), `Franchise`/`Contrat de franchise` (retrait distributeur / enseigne, ajout franchise alimentaire) ; `REP emballages` + variantes (`Responsabilité élargie du producteur emballages`, `Consigne des emballages`, `Consigne emballage`) passés en contextual avec marqueurs GD-alimentaires uniquement (faux positif batteries lithium). Validation 10/10 bons matchs OK + 10/10 faux positifs filtrés.
  2. **Red list** — nouveau fichier `config/redlist.yml` listant 19 URLs canoniques explicitement exclues du site et du digest. Mécanisme `_filter_redlisted` dans `site_export.py`, appliqué juste après `_filter_disabled_sources`. Strippe scheme + fragment text-fragment + trailing slash pour comparer. Couvre les cas où le faux positif persiste malgré un lexique sain (homonymie « char Leclerc » sur PJL militaire, fiche député qui mentionne incidemment un nom d'enseigne, etc.).
  3. **Titres CR plénière AN simplifiés** — retrait du thème (1er point d'ODJ) dans les titres « Séance du JJ/MM/AAAA — … ». Le thème extrait par `_SYCERON_THEME_RE` correspondait quasi systématiquement au PREMIER point de l'ordre du jour, alors que l'occurrence keyword qui justifie l'inclusion pouvait être beaucoup plus loin → titre trompeur. On reste désormais sur « Séance AN du JJ/MM/AAAA — Compte rendu intégral ». Le snippet (centré sur le 1er match) + le text-fragment dans l'URL fournissent le contexte précis. Modifié dans `assemblee.py::_fetch_xml_zip` ET dans `site_export._fix_cr_row` (qui aurait sinon réécrit le titre avec le thème pour les CR pré-patch).

  Confirmé en analyse code : l'ADLC (source `autorite_concurrence`) n'est PAS dans `BYPASS_KEYWORDS_SOURCES` (vide), donc déjà filtrée par les keywords. Le retrait de « Autorité de la concurrence » du direct résout par effet de bord son flot de bruit (les communiqués ADLC qui matchaient juste sur ce keyword tombent ; ceux avec un autre signal GD authentique passent toujours).

- 2026-04-26 (soir, série de fixes post-merge PR #1) : 4 fixes successifs après signalements Cyril.
  1. `a2881b9` — bug « 0 CR AN matché sur 6 mois ». Troncature haystack 10 k chars (an_cr_commissions) + 2 k chars (an_syceron via summary). Sur 9 CR cion-eco témoins, kw GD au-delà de pos 10 000 → invisibles. Fix : 200 k chars partout. Validation 9/9 CR matchent + 24/50 séances plénières vs 1/50.
  2. `ae273ee` — workflow `reset_db=1` ne purgeait pas `data/an_cr_state.json` → CR commission pré-reset jamais ré-ingérés. Fix : `rm -f data/an_cr_state.json` ajouté au step Reset SQLite.
  3. `14f0bc6` — bug « 2 propositions de loi 2025 avec URL pointant vers textes 1977/1986 ». `uid="247"` pas unique (9 lignes CSV même numéro). store.upsert_many garde l'url immuable → la 1re ligne (la plus ancienne) impose l'URL. Fix : `uid = slug URL` (`ppl24-247`) + status_label depuis « État du dossier ».
  4. `3657ac5` — port Bump R40-I sport : snippet centré sur 1er matched_keyword (et plus seulement sur un context marker quelconque). `apply()` choisit en plus entre summary et haystack_body selon où se trouve le kw — résout le cas des CR AN dont l'extrait affiché ne contenait pas la phrase ayant déclenché le match.

  Run #41 lancé sur `3657ac5` (`reset_db=1, since_days=180, no_email=1`) pour propager les 4 fixes. Runs #34 / #36 / #38 / #39 obsolètes.
